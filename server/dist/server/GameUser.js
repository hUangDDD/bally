...
const GameItemDefine = require("../shared/GameItemDefine");
const objectutil = require("./objectutil");
const PetRules = require("../shared/PetRules");
const DT = require("../shared/DailyTaskDefine");
const WT = require("../shared/WeeklyTaskDefine");
const PetSkillDesc = require("../shared/PetSkillDesc");
const GD = require("../shared/GiftDefine");
const PD = require("../shared/PaymentDefine");
const cash = require("./cash");
const jrpc = require("./jrpc");
const util = require("./util");
const HighScoreAward = require("../shared/HighScoreAward");
const RankListDB = require("./RankListDB");
const MatchRules = require("../shared/MatchRules");
const ENABLE_DAILY_TASK = false;
class GameUser {
    constructor(dbuser, socket, server) {
        this._closed = false;
        //private _currentGame: any;//当前开始的游戏。
        this._loginSent = false; //是不是发送过了login成功
        this.matchPlayer = null; //如果matchPlayer !== null && matchGame === null表示正在匹配中
        this.matchGame = null;
    ...
    onMessage(obj) {
        ...
        switch (cmd) {
            ...
            case 'match_ready':
                if (this.matchGame)
                    this.matchGame.setReady(this);
                break;
            case 'match_score':
                if (this.matchGame)
                    this.matchGame.setGameScore(this, obj);
                break;
    ...
    recvEnterMatch(obj) {
        if (this.matchPlayer || this.matchGame)
            return;
        var sendError = (msg, nocoin) => {
            this.socket.sendPacket({ cmd: 'enter_match_error', msg: msg, nocoin: nocoin });
        };
        var type = obj.type;
        if (['11', '44', 'master'].indexOf(type) < 0)
            return;
        if (this.matchGame || this.matchPlayer)
            return;
    ...
    recvReqStartGame(obj) {
        ...
        let sendStartGameError = (msg, nocoin, need) => {
    ...
    processGameResult(obj, currentGame, outputGameOver) {
        let sendError = msg => {
        ...
        if (!isTutorial && !isMatch) {
            ...
            if (obj.maxCombo > db.maxCombo)
                db.maxCombo = obj.maxCombo;
            if (obj.maxLink > db.maxLink)
                db.maxLink = obj.maxLink;
            db.totalKill += totalBall;
            this._updateDailyTask(taskData);
            let weeklyTaskOutput = {};
        ...
        let goObject = {
            ...
            petResult: petResult,
            pet: mainPet,
            petScore: mainPetScore,
            petAddPercent: petAddPercent,
            petAddScore: petAddScore,
            itemAddPercent: itemAddPercent,
            itemAddScore: itemAddScore,
            coin: totalCoin,
            score: totalScore,
        ...
        if (weekHighScoreChanged)
            goObject.weekHighScoreChanged = weekHighScoreChanged;
        if (historicalHighScoreChanged)
            goObject.historicalHighScoreChanged = historicalHighScoreChanged;
        if (weeklyTaskSatisfied)
            goObject.weeklyTaskSatisfied = true;
    ...
    sendLoginSuccess(loginObject) {
        ...
        if (!this.dbuser.eventTags)
            this.dbuser.eventTags = {};
    ...
    */
    addMail(type, count, text, extraData) {
        let db = this._dbUser;
        let mail = {
            id: db.nextMailId | 0,
            type: type,
    ...
    /**给这个用户发一分请求加好友的邮件 */
    addAddFriendMail(key, text, faceurl) {
        let db = this._dbUser;
        let mail = {
            id: db.nextMailId | 0,
            type: 'addFriend',
    ...
    _applyMail(mail) {
        ...
        if (type === 'heart' && mail['_friendheart']) {
            ...
            if (db.recvHeartCount >= 50)
                return; //收了超过50个，则直接return
    ...
    _processHistoricalScoreAward(oldScore, newScore) {
    ...
    _updateWeeklyTask(taskData, output) {
        let db = this._dbUser;
        if (!db.weeklyTasks)
            return;
        let currentTask = db.weeklyTasks[db.weeklyTasks.length - 1];
            ...
            if (task) {
                if (task.status === 'satisfied') {
                    ...
                    if (task.prizeType && task.prizeCount > 0) {
                        ...
                        this.addMail(task.prizeType, task.prizeCount, `完成冒险"${task.desc}"奖励${task.prizeCount}${name}`);
                    }
                }
            }
        }
    ...
    _addWeeklyTask(callSendWeeklyTask) {
        let changed = false;
        let sysWTTask = WT.getCurrentWeeklyTaskSet();
        let db = this._dbUser;
        if (!sysWTTask) {
    ...
    _recvBuyGift(obj) {
        let sendBuyError = (msg, nocoin, need) => {
            this.socket.sendPacket({
                cmd: 'buyGiftError',
                msg: msg,
                nocoin: nocoin,
    ...
    */
    _initFriends(obj) {
        ...
        if (obj && typeof obj.from === 'string' && obj.from !== this.key && this._findFriendInDB(obj.from) < 0) {
            this._server.requestAddFriend(this, obj.from, (success) => {
                if (success)
                    util.safePush(this.dbuser, 'friends', { key: obj.from });
    ...
    _recvBuyCoin(obj) {
        let sendError = (msg, nodiamond, need) => {
            this.socket.sendPacket({ cmd: 'buyCoinError', msg: msg, nodiamond: nodiamond, need: need });
        };
    ...
    _recvBuyHeart(obj) {
        let sendError = (msg, nodiamond, need) => {
            this.socket.sendPacket({ cmd: 'buyHeartError', msg: msg, nodiamond: nodiamond, need: need });
        };
        if (typeof obj.item !== 'string')
            return;
...
function isSameDay(t1, t2) {
    if (Math.abs(t2 - t1) > 24 * 3600 * 1000)
        return false;
    let d1 = new Date(t1);
    let d2 = new Date(t2);

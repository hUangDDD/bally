...
import * as builder_api from "./builder.api"
import * as builder from "./builder"
import * as cash from "./cash"
import * as bodyParser from "body-parser";
import * as jrpc from "./jrpc"
import * as ManagerApi from "./ManagerApi"
...
function getLocalIp()
{
	function isPrivateIp(ip)
	{
		...
		if (s0 == 10) return true;
		if (s0 == 172 && s1 >= 16 && s1 < 31) return true;
		if (s0 == 192 && s1 == 168) return true;
	}
	...
	for (var k in interfaces)
	{
		if (typeof k === 'string')
		{
			...
			for (var i = 0; i < localDevice.length; i++) if (k_small.indexOf(localDevice[i]) >= 0) break;
			if (i < localDevice.length) continue;
		}
	...
	{
		if (addresses[i]) return addresses[i];
	}
	return p[0];
}

...
type WeeklyTaskType =
	...
	"totalScore" |
	"totalBallX" |   //累计消除宠物X
	"totalSkillX"   //累计使用果冻技能X

export const ALL_WEEKLY_TASK_TYPE = [
	"gameCombo",
	"gameExp",
	"gameCoin",
	"gameBomb",
...
export function splitWeeklyTaskType(type: string, param)
{
	let prefix: string;
	let tail: string;
...
export interface IWeeklyTaskDefine
{
	type: WeeklyTaskType;
	maxCount: number; //累计型的累计数量，不支持的话，则是1；如果是单据类型的，则表示需要在单据内达到的数量
	failCount: number;//保底次数，0表示不支持保底
	prizeType?: 'coin' | 'diamond' | 'heart';
	prizeCount?: number;
	desc: string;
}

export interface IWeeklyTaskSet
{
	id: any;
	from: number;
	to: number;
	tasks: IWeeklyTaskDefine[];
}
var TASK_SETS: IWeeklyTaskSet[];

export function getCurrentWeeklyTaskSet(): IWeeklyTaskSet 
{
	let now = Date.now();
	if (TASK_SETS)
	{
...
//这是一个标准的模板
const TASK_LIST1: IWeeklyTaskDefine[] = [
...
var TASK_LIB: IWeeklyTaskDefine[]/* = [
	null,
...
/*
const STD_TASK_LIST1: IWeeklyTaskDefine[] = [
	TASK_LIB[5],
	TASK_LIB[7],
	TASK_LIB[20],
	TASK_LIB[4],
	merge(TASK_LIB[1], { prizeType: 'heart', prizeCount: 5 }),
	TASK_LIB[6],
	TASK_LIB[14],
	TASK_LIB[2],
	TASK_LIB[17],
	merge(TASK_LIB[3], { prizeType: 'diamond', prizeCount: 5 }),
	TASK_LIB[19],
	TASK_LIB[8],
	TASK_LIB[41],
	TASK_LIB[30],
	merge(TASK_LIB[34], { prizeType: 'coin', prizeCount: 10000 }),
	TASK_LIB[26],
	TASK_LIB[24],
	TASK_LIB[53],
	TASK_LIB[56],
	merge(TASK_LIB[86], { prizeType: 'coin', prizeCount: 20000 }),
];


const TASK_SET1: IWeeklyTaskSet = {
	id: "123451",
	from: Date.parse("2016-4-29"),
	to: Date.parse("2016-5-29"),
	tasks: STD_TASK_LIST1
};
*/
...
if (typeof process === 'object')
{
	...
	//读取所有任务们
	TASK_SETS = [];
	var currentTaskSet: IWeeklyTaskSet;
	...
	for (let i = 0; i < task_records.length; ++i)
	{
		...
		if (line[0] === 'id')
		{
			if (currentTaskSet) TASK_SETS.push(currentTaskSet);
			currentTaskSet = {
				id: line[1],
...
else
{
	...
	if (!TASK_SETS) alert('error load weekly task');
}
